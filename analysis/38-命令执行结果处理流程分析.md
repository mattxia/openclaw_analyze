...
```mermaid
flowchart TB
    subgraph "入口层 - Agent 工具调用"
        A[Agent 调用 exec 工具]
    end

    subgraph "参数解析层 - bash-tools.exec.ts"
        B[参数验证<br/>validateScriptFileForShellBleed]
        C[解析执行配置<br/>host/security/ask/pty]
        D[环境变量处理<br/>sanitizeHostBaseEnv]
    end

    subgraph "审批层 - exec-approvals.ts"
        E{需要审批?}
        F[构建审批消息<br/>buildApprovalPendingMessage]
        G[等待用户审批]
    end

    subgraph "执行层 - bash-tools.exec-runtime.ts"
        H[创建 ProcessSession]
        I[添加进程到注册表<br/>addSession]
        J{沙箱模式?}
        K[使用 docker exec]
        L[使用本地 shell]
    end

    subgraph "进程管理 - supervisor.ts"
        M[createProcessSupervisor]
        N[生成 runId]
        O[创建 RunRecord]
        P[设置超时定时器]
        Q[spawn 创建子进程]
    ...
    subgraph "适配器层 - adapters/"
        R[createChildAdapter<br/>子进程模式]
        S[createPtyAdapter<br/>PTY 终端模式]
    end

    subgraph "底层执行 - Node.js APIs"
        T[child_process.spawn]
        U[node-pty spawn]
    end

    subgraph "输出处理 - bash-process-registry.ts"
        V[捕获 stdout/stderr]
        W[appendOutput<br/>累积输出]
        X[处理截断]
    end

    subgraph "结果判断 - exec-runtime.ts"
        Y{wait 等待退出}
        Z{exitCode == 0?}
        AA{shell 失败码<br/>126/127?}
        AB[status = completed]
        AC[status = failed]
        AD[超时检测]
        AE[信号中止检测]
    ...
    subgraph "返回结果 - AgentToolResult"
        AF[成功结果<br/>content + exitCode]
        AG[失败结果<br/>reason + exitCode]
        AH[运行中结果<br/>sessionId + pid]
    end
